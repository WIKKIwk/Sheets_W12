...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ size = 300 }) => {
  ...
  const maxRadius = size * 0.43; // Slightly larger outer circle (43% of size)

  // Bar styling (scaled to size)
  const barWidth = size * 0.04; // 4% of size
  const barGap = size * 0.02; // 2% of size
  const cornerRadius = size * 0.02; // 2% of size
  ...
  for (let i = 0; i < numCols; i++) {
    ...
    const maxGapHalfHeight = size * 0.22; // Taller gap in the center (22% of size)
